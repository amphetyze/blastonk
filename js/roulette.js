...
function createSkinItems() {
    ...
    allSkins.forEach((skin) => {
      ...
      <div class="roulette-bg">
        ...
        <img src="https://sap-sap.ru/img/skins/${skin.id}.png" alt="${
            skin.title
        }" class="skins__image" />
...
function spinRoulette() {
    ...
    skinsContent.style.transform = `translateX(-${
        stepsToScroll * 233 - 327 - 44 + marginLeft
    }px)`
...
function openModal(skin) {
    ...
    const button = document.querySelector(".roulette__button")
    const rect = button.getBoundingClientRect()
    window.scrollTo({
        top: rect.top + window.scrollY - window.innerHeight + 88,
    })
    ...
    const skinElement = document.createElement("div")
    skinElement.innerHTML = `
      <div class="roulette-bg-win">
        <div class="skins__rarity" style="background: ${
            rarityColor[skin.rarity]
        };"></div>
        <img src="https://sap-sap.ru/img/skins/${skin.id}.png" alt="${
        skin.title
    }" class="skins__image" style="width: 190px; height: 133px;" />
        <div class="skins__amount">$ ${skin.price.toLocaleString("ru-RU")}</div>
        <p class="skins__title">${skin.title}</p>
      </div>`
